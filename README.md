...
4 パッケージは足並みを揃えて上げます。`ui-core` と `ui-vue` が他を `^` で要求しており、
別々に動かすと組み合わせの検証範囲が広がるだけで、得るものがないためです。

タグは `<パッケージ名>@<版>`（例 `ui-core@0.1.0`）。**publish が通ってから打ちます。**
先に打つと、公開までのあいだに入った変更でタグと中身がずれます
（実際に一度ずらして、版を飛ばして出し直しました）。
タグがあるものは npm にある、と読めるようにしておきます。

`0.x` のあいだは公開 API が変わりえます（spec の `platforms` が `stable` になるまで）。
